...
| **mem-on v1**(当前可比最好) | **45.64** | noise-inclusive | 见 §1,**记忆净负 ~4.2**;失败=接口错配 |
| mem-on v2 | 46.9 | **no-noise(8429)** ⚠️ | 见 §2,**口径不同,不可与上面直接比** |
| mem-on v3(全解冻冷启动) | 17.5(峰值,PILOT) | noise-inclusive(PILOT 200/点) | 见 §5,**崩**;失败=训练/rollout 泛化裂缝 |


> **⚠️纠正:** v2 的 46.9 是在 **不含 noise 的 8429 集** 上算的,
> 跟 49.83 / 45.64 的 noise-inclusive 口径**不可比**。所以 v2 的"+1.3"是假象,
> **当前真正可比的最好成绩仍是 v1 的 45.64**。渐进解冻并未被证明真的超过 v1。



---

## 1. v1 — 只训 VAE temporal 4 参数(DiT 全冻)
...
1. **不含 noise 的 8429 集** 上算的,与 49.83 /
   45.64 的 noise-inclusive(10030)口径不一致。需要重新跑libero plus含noise，但可以确定的是比v1更差
...
如果我们要在1.2s之内，就用H4吧，这也是我v3用的


---

## 5. v3 — 从 base 冷启动,全 DiT + temporal 联合 co-adapt(H4)

> 思路转向:**不再在 v1/v2 的歪地基上打补丁**。从 base 原版 ckpt 冷启动,
> 整个 DiT 自由适应带记忆的 latent,从一开始就 co-adapt。更改方向：历史砍到 H4。

| 项 | 值 |
|---|---|
| 脚本 | `scripts/train_mem_stage1_v3.sh`(`HISTORY` 参数化,默认 4) |
| run 目录 | `runs/mem_temporal_libero_stage3/` |
| 起点 ckpt | `checkpoints/fastwam_release/libero_uncond_2cam224.pt`(base 原版,mem-off 49.83) |
| resume | weights-only 冷启动(不继承 v1/v2);有 DeepSpeed state 则续(须保持 8 卡) |
...
| lr | **1e-5**,cosine + 5% warmup(全解冻 blast radius 大,比 v2 的 3e-5 更保守) |
...
缩放未必修得动,全解冻自由度才够真正 co-adapt。**全解冻不贵**:v2 的 backward
...
> 峰值 17.5 也只有 v1 45.64 的零头。**Camera / BG 全程 0**,goal/long 类接近 0 = 典型 rollout 不稳。

**失败原因(全解冻冷启动 = 训练/rollout 泛化裂缝,不是权重损毁):**
- 训练 loss **健康**:warmup 起点 2.18(action 1.21/video 0.96)→ 收敛到 **0.178**(action 0.038/
  video 0.14),与 v1 的 ~0.10 同量级(略高由 H4 上下文更短解释)。加载日志正常
...
- 但 eval 成功率崩到 2-17.5%。**训练 loss 健康 + eval 崩 = 训练/rollout 泛化裂缝**:全解冻让
  ...
  bias / 误差累积)。v1/v2 冻死 DiT,走不上这条路,locked DiT 保住了 base 的 rollout 稳定性。
...
v2 step3000 @H16、v3 step4000 @H4。base=95.9。判读:**崩到 10-40% → rollout 裂缝坐实**;
...
| 内容 | 路径 |
|---|---|
| v1 脚本 / run | `scripts/train_mem_stage1_v1.sh` / `runs/mem_temporal_libero/` |
| v2 脚本 / run | `scripts/train_mem_stage1_v2.sh` / `runs/mem_temporal_libero_stage2/` |
| v3 脚本 / run | `scripts/train_mem_stage1_v3.sh` / `runs/mem_temporal_libero_stage3/` |
