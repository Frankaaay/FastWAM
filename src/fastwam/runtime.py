...
def create_fastwam(
    ...
    return FastWAM.from_wan22_pretrained(
        ...
        mot_torch_compile=bool(mot_torch_compile),
        mot_torch_compile_mode=str(mot_torch_compile_mode),
        vae_torch_compile=bool(vae_torch_compile),
        vae_torch_compile_mode=str(vae_torch_compile_mode),
        vae_encode_functional=bool(vae_encode_functional),
        vae_encode_functional_mode=str(vae_encode_functional_mode),
...
def run_inference(cfg: DictConfig):
    ...
    action_horizon = int(inference_cfg.action_horizon)
    ...
    infer_out = model.infer_action(**infer_kwargs)
    action = infer_out["action"]
    output_action_path = inference_cfg.get("output_action_path")
    if output_action_path:
        torch.save({"action": action}, str(output_action_path))
        logger.info("Saved inference action to %s", output_action_path)
        return str(output_action_path)
    logger.info("Finished action-only inference with action shape %s", tuple(action.shape))
    return action
