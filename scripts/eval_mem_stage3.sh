#!/bin/bash
# ----------------------------------------------------------------------------
# eval_mem_stage3.sh — MEM-stage3(fold-current 路线)eval 包装。
#
...
#   - HISTORY=5   -> H5(4n+1,K_lat=2),与 train_mem_stage3.sh 对齐;
#   - INCLUDE_NOISE=1(libero_plus 时)-> 全 10030,对齐 paper(项目硬规则)。
#
# 用法:
#   bash scripts/eval_mem_stage3.sh                 # 自动选 runs/mem_stage3 最新 weights ckpt
#   STEP=4000 bash scripts/eval_mem_stage3.sh       # 指定档位(weights/step_004000.pt)
#   CKPT=/path/to.pt bash scripts/eval_mem_stage3.sh# 显式指定 ckpt
#   PILOT=16 bash scripts/eval_mem_stage3.sh        # ⚠️ 全量前先跑:验证 fold 推理链路通
#   NUM_GPUS=4 GPU_OFFSET=4 ...                         # 训练还占着 0-3 时,把 eval 钉在 4-7
#   BENCH=libero bash scripts/eval_mem_stage3.sh    # 原版未扰动标准 LIBERO(基线 95.9,默认 TRIALS=5)
#
# ⚠️ 第一次评 stage3 先 PILOT=16:smoke 只验证训练 forward,推理 rollout 的 fold 路径
...
RUN_DIR=${RUN_DIR:-runs/mem_stage3}
...
[ -n "${CKPT:-}" ] && [ -f "$CKPT" ] || {
  echo "[FATAL] 找不到 stage3 ckpt(CKPT=${CKPT:-未解析})。"
...
if [ "$BENCH" = "libero" ]; then
  ...
  export OUT=${OUT:-./evaluate_results/libero_std/mem_stage3/${STEP_TAG}_$(date +%Y%m%d_%H%M%S)}
  BASELINE="对照基线 标准 libero 95.9"
else
  export INCLUDE_NOISE=1           # 硬规则:libero-plus eval 必须全 10030
  export OUT=${OUT:-./evaluate_results/libero_plus/mem_stage3/${STEP_TAG}_$(date +%Y%m%d_%H%M%S)}
  BASELINE="对照基线 mem-off 50.5"
fi

echo "[stage3 eval] BENCH=$BENCH CKPT=$CKPT"
echo "[stage3 eval] VAE_MEM=false DIT_PREPEND=false DIT_FOLD_CURRENT=true HISTORY=$HISTORY TRIALS=${TRIALS:-1} INCLUDE_NOISE=${INCLUDE_NOISE:-0} PILOT=${PILOT:-0}"
echo "[stage3 eval] OUT=$OUT  $BASELINE"
